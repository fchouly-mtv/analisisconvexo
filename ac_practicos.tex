...
% ========== Ejercicios en 
\section{Dimensión infinita} \begin{ex}[Norma en espacio de Hilbert]
...
\begin{solucion}
    Para cualquier $u, v \in H$ y $\lambda \in [0, 1]$, se tiene:
    \[
    f(\lambda u + (1-\lambda)v) = \int_0^1 e^{\lambda u(x) + (1-\lambda)v(x)} \, dx \leq \int_0^1 \left( \lambda e^{u(x)} + (1-\lambda) e^{v(x)} \right) \, dx = \lambda f(u) + (1-\lambda) f(v),
    \]
    donde la desigualdad sigue de la convexidad de la función exponencial $e^t$ en $\mathbb{R}$. Por lo tanto, $f$ es convexa.
\end{solucion}

---

\begin{ex}[Función de energía en $H^1_0(\Omega)$]
    Sea $\Omega \subseteq \mathbb{R}^n$ un dominio acotado y $H = H^1_0(\Omega)$ el espacio de Sobolev de funciones con traza nula. Define $f: H \to \mathbb{R}$ como:
    \[
    f(u) = \frac{1}{2} \int_\Omega |\nabla u(x)|^2 \, dx.
    \]
    Demuestra que $f$ es convexa y diferenciable, y calcula su gradiente.
\end{ex}

\begin{solucion}
    La convexidad sigue de la linealidad del operador gradiente y la convexidad de la función $t \mapsto t^2$:
    \[
    f(\lambda u + (1-\lambda)v) = \frac{1}{2} \int_\Omega |\nabla (\lambda u + (1-\lambda)v)|^2 \, dx = \frac{1}{2} \int_\Omega |\lambda \nabla u + (1-\lambda) \nabla v|^2 \, dx \leq \frac{\lambda}{2} \int_\Omega |\nabla u|^2 \, dx + \frac{1-\lambda}{2} \int_\Omega |\nabla v|^2 \, dx = \lambda f(u) + (1-\lambda) f(v).
    \]
    Para el gradiente, calculamos la derivada direccional:
    \[
    \langle \nabla f(u), h \rangle = \lim_{t \to 0} \frac{f(u + th) - f(u)}{t} = \int_\Omega \nabla u \cdot \nabla h \, dx = \langle -\Delta u, h \rangle_{L^2},
    \]
    donde $-\Delta u$ es el Laplaciano de $u$ con condiciones de Dirichlet. Por lo tanto, $\nabla f(u) = -\Delta u$.
\end{solucion}

---

\begin{ex}[Función de entropía en $L^1$]
    Sea $(\Omega, \mathcal{F}, \mu)$ un espacio de medida con $\mu(\Omega) = 1$, y sea $L^1_+(\Omega)$ el conjunto de funciones no negativas en $L^1(\Omega)$. Define $f: L^1_+(\Omega) \to \mathbb{R}$ como:
    \[
    f(u) = \int_\Omega u(x) \log u(x) \, d\mu(x),
    \]
    donde $0 \log 0 = 0$. Demuestra que $f$ es convexa.
\end{ex}

\begin{solucion}
    La función $t \mapsto t \log t$ es convexa en $\mathbb{R}_+$ (su segunda derivada es $1/t \geq 0$). Por el teorema de Jensen para integrales, para cualquier $u, v \in L^1_+(\Omega)$ y $\lambda \in [0, 1]$, se tiene:
    \[
    f(\lambda u + (1-\lambda)v) = \int_\Omega [\lambda u(x) + (1-\lambda)v(x)] \log [\lambda u(x) + (1-\lambda)v(x)] \, d\mu(x) \leq \lambda \int_\Omega u(x) \log u(x) \, d\mu(x) + (1-\lambda) \int_\Omega v(x) \log v(x) \, d\mu(x) = \lambda f(u) + (1-\lambda) f(v),
    \]
    donde la desigualdad sigue de la convexidad de $t \log t$. Por lo tanto, $f$ es convexa.
\end{solucion}

---

\begin{ex} %[Función de norma en $L^p[0,1]$]
    Sea $1 < p < \infty$ y $X = L^p[0,1]$. Demuestra que la función $f: X \to \mathbb{R}$ definida por $f(u) = \|u\|_p^p$ es estrictamente convexa.
\end{ex}

\begin{solucion}
    Para cualquier $u, v \in X$ con $u \neq v$ y $\lambda \in (0, 1)$, se tiene:
    \[
    f(\lambda u + (1-\lambda)v) = \|\lambda u + (1-\lambda)v\|_p^p = \int_0^1 |\lambda u(x) + (1-\lambda)v(x)|^p \, dx.
    \]
    Por la desigualdad estricta de Jensen para la función $t \mapsto |t|^p$ (que es estrictamente convexa en $\mathbb{R}$), tenemos:
    \[
    |\lambda u(x) + (1-\lambda)v(x)|^p < \lambda |u(x)|^p + (1-\lambda)|v(x)|^p \quad \text{c.s. en } [0,1],
    \]
    donde la desigualdad es estricta en un conjunto de medida positiva (ya que $u \neq v$). Integrando, obtenemos:
    \[
    f(\lambda u + (1-\lambda)v) < \lambda f(u) + (1-\lambda) f(v),
    \]
    lo que demuestra que $f$ es estrictamente convexa.
\end{solucion}
