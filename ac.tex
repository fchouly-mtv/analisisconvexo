...
\title{Análisis convexo y métodos numéricos}
...
Agradezco al CMAT, a FCIEN, a Pedeciba y al IFUMI. Tambi\'en por sus consejos y apoyo:
Dominique Attali,
...
\begin{theorem}
%\textbf{Teorema.} 
Si $\{u_n\}$ converge débilmente hacia $u$ en un espacio normado $V$, entonces $\{u_n\}$ está acotada.
\end{theorem}
...
Mejorar aqui con Banach-Steinhaus +++
\textbf{Demostración.} Por el {Principio de acotación uniforme} (o Teorema de Banach-Steinhaus), si $(v_n)$ converge débilmente, entonces el conjunto $\{v_n : n \in \mathbb{N}\}$ es puntualmente acotado en $V^*$. Es decir, para todo $f \in V^*$, existe $M_f > 0$ tal que:
\[
|f(v_n)| \leq M_f \quad \forall n \in \mathbb{N}.
\]
Por el {Teorema de Banach-Steinhaus}, esto implica que $\{v_n\}$ está acotado en norma:
\[
\exists M > 0 \text{ tal que } \|v_n\| \leq M \quad \forall n \in \mathbb{N}.
\]
...
\textbf{Teorema.} Si $(v_n)$ converge débilmente hacia $v$ en un espacio normado $V$, entonces:
\[
...
\| v \|_V =
\frac{(v,v)_V}{\| v \|_V}
= \lim_n \frac{(v_n,v)_V}{\| v \|_V}
\]
y luego
\[
\frac{(v_n,v)_V}{\| v \|_V} \leq \|v_n\|_V
\]
y finalmente
\[
\liminf \frac{(v_n,v)_V}{\| v \|_V} \leq \liminf \|v_n\|_V
\]
%\[
%\| v \|_V =
...
3. Existencia de una subsucesión que converge débilmente:
   Por el Lema anterior, existe una subsucesión \(\{u_{n_k}\}\) que converge débilmente a algún \(u \in V\). Además, por el Lema anterior, como \(K\) es cerrado y convexo, \(u \in K\).

4. \(J(u)\) es el mínimo:
   Por el Lema  anterior, \(J\) es s.c.i. para la convergencia débil. Por lo tanto:
   \[
...
\noindent Ahora necesitamos los siguientes resultados para caracterizar el mínimo con la diferencial de $J$. Primero adaptamos la Proposición 10.1.4 de Allaire.
...
\begin{theorem}
    Si $K$ es un subconjunto convexo cerrado no vacío de $V$ y $\mathcal{J}$ es convexa y derivable en $V$, entonces $u$ es un minimizador global de $\mathcal{J}$ en $K$ si y solo si
    \[
...
\begin{proof}
Primero suponemos que $u$ es un minimizador global de $\mathcal{J}$ en $K$, entonces para $\varepsilon > 0$, se tiene, para $v \in K$:
\[
...
Supongamos ahora que se cumple la DV. Usamos la Proposición anterior en $u$: para todo $v \in K$:
\[
...
\section{Recuerdo de Newton suave}
...
\section{Hacia Newton semi-suave}

\dots

\section{Mas sobre funciones convexas}

Tambien, semi continuidad inferior y continuidad de las funciones convexas.
