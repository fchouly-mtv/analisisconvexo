...
%\section{Un ejemplo triste}
Consideremos la función convexa $f(x) = x^2 + 2x + 1$. El objetivo es encontrar el mínimo global de esta función utilizando el método de descenso de gradiente.

\subsection{Definición}
El método de \textbf{descenso de gradiente a paso constante} se define como:
$$
\mathbf{x}_{k+1} = \mathbf{x}_k - \alpha \nabla f(\mathbf{x}_k)
$$
donde:
\begin{itemize}
    \item $\mathbf{x}_k$ es la posición actual en la iteración $k$,
    \item $\alpha$ es el \textbf{tamaño del paso} (constante),
    \item $\nabla f(\mathbf{x}_k)$ es el gradiente de la función $f$ evaluado en $\mathbf{x}_k$.
\end{itemize}
...
\section{Gradiente a paso variable}

\subsection{Definición}
En el método de \textbf{descenso de gradiente a paso variable}, el tamaño del paso $\alpha_k$ puede cambiar en cada iteración. Una estrategia común es usar una \textbf{búsqueda de línea} (line search) para elegir $\alpha_k$ que minimice $f(\mathbf{x}_k - \alpha_k \nabla f(\mathbf{x}_k))$.
%
...
\begin{lstlisting}[style=pythonstyle, caption={Gradiente a paso variable.}, label={lst:g2}]
from scipy.optimize import minimize_scalar

def gradient_descent_variable(x0, max_iter=1000):
    x = x0
    for _ in range(max_iter):
        grad = df(x)
        # Búsqueda de línea para encontrar alpha óptimo
        res = minimize_scalar(lambda a: f(x - a * grad), bounds=(0, 1), method='bounded')
        alpha_k = res.x
        x = x - alpha_k * grad
    return x
...
\begin{itemize}
    \item Un conjunto $C \subset V$ es \textbf{convexo} si para todo $x, y \in C$ y todo $\lambda \in [0, 1]$, se cumple:
    $$
    \lambda x + (1 - \lambda) y \in C.
    $$
    \item Una función $f: V \to \mathbb{R} \cup {+\infty}$ es \textbf{convexa} si para todo $x, y \in V$ y todo $\lambda \in [0, 1]$, se cumple:
    $$
    f(\lambda x + (1 - \lambda) y) \leq \lambda f(x) + (1 - \lambda) f(y).
    $$
    \item Funci\'on convexa fuerte:
%    * Strongly convex function 911 (definition): 
\begin{equation}
    J(\frac12 (u+v)) \leq \frac12 (J(u)+J(v)) - \frac{\alpha}{8} \| u-v \|^2.
\end{equation}
%(it is convex and strictly convex).
\end{itemize}

\subsection{Ejemplo}
Sea $V = L^2([0,1])$ el espacio de Hilbert de funciones cuadrado-integrables en $[0,1]$.
La función $f(u) = \int_0^1 |u(t)|^2 , dt$ es convexa en $V$.
...
Sea $K \subset V$ un conjunto convexo y cerrado, y $f: V \to \mathbb{R}$ una función convexa y diferenciable.
Un punto $u \in K$ es mínimo de $f$ en $K$ si y solo si:
%
$$
...
\section{Desigualdad variacional}
