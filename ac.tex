...
Sea \( V \) un espacio de Hilbert separable con producto interno \( (\cdot, \cdot)_V \) y norma inducida \( \|\cdot\|_V \), y sea \( \{e_i\}_{i \in \mathbb{N}} \) una {base hilbertiana} (ortonormal) de \( V \). Entonces:

\begin{enumerate}
    \item \textbf{Descomposición ortonormal:}
    ...
    \item \textbf{Identidad de Parseval:}
    Para todo \( u, v \in V \):
    \[
...
\textbf{Teorema.} Si $(v_n)$ converge fuertemente hacia $v$ en un espacio normado $(V, \|\cdot\|)$, entonces $(v_n)$ converge débilmente hacia $v$.

\medskip

\textbf{Demostración.} Por hipótesis, $v_n \to v$ fuertemente, es decir, $$\lim_{n \to \infty} \|v_n - v\| = 0.$$ Sea $f \in V^*$ (el dual de $V$). Como $f$ es continua (por ser lineal y acotada), se tiene:
\[
|f(v_n) - f(v)| = |f(v_n - v)| \leq \|f\| \cdot \|v_n - v\| \xrightarrow[n \to \infty]{} 0.
\]
Por lo tanto, $f(v_n) \to f(v)$ para todo $f \in V^*$, lo que significa que $v_n \rightharpoonup v$ débilmente.

---

\subsubsection{2La convergencia débil implica acotación}

\textbf{Teorema.} Si $(v_n)$ converge débilmente hacia $v$ en un espacio normado $V$, entonces $(v_n)$ está acotada.
