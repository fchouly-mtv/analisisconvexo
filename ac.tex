...
\section{Caso estrictamente convexo}
...
Funcional de energía asociado a una EDP elíptica no lineal (superficie mínima). 
   Consideremos el funcional \(\mathcal{J}: H_0^1(\Omega) \to \mathbb{R}\) definido por:
   \[
   \mathcal{J}(v) = \int_\Omega \left( \sqrt{1 + |\nabla v|^2} - 1 \right) - \int_\Omega f v,
   \]
...
\emph{Implicación inversa (\(\Leftarrow\)):}
  ...
  \]
  Por hipótesis, \(\langle u_n, e_i \rangle_V \to \langle u, e_i \rangle_V\) para cada \(i\). Además, por el Teorema de Banach-Steinhaus~\cite{brezis2011}, \(\{u_n\}\) es acotada (pues converge débilmente). Entonces, por el Teorema de Lebesgue dominado, podemos intercambiar el límite y la suma:
  \[
...
\begin{ex}
En $L^2(0;\pi)$, la secuencia de funciones $\{f_n\}$ definidas por
\[
f_n(x)=\sen^2(n x)
\]
verifica
\[f_n \rightharpoonup \frac12\]
pero no converge fuertemente, ver los detalles en 
\cite{bressan2014}.
\end{ex}
...
\begin{proof}
Viene directamente de la definición de s.c.i.
\end{proof}
